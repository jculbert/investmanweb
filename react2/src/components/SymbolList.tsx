...
export function SymbolList() {
  ...
  const current = selectedSymbol ? editedSymbol || selectedSymbol : null;
  const hasChanges = selectedSymbol !== null && editedSymbol !== null && JSON.stringify(editedSymbol) !== JSON.stringify(selectedSymbol);
  ...
  const handleSave = async () => {
    if (!hasChanges || !editedSymbol || !selectedSymbol) return;

    try {
      setSaving(true);
      setSaveError(null);
      await updateSymbol(selectedSymbol.name, editedSymbol);
      setSymbols((existing) => existing.map((symbol) => (symbol.name === editedSymbol.name ? editedSymbol : symbol)));
  ...
  const renderLoading = () => <div className="symbol-list loading">Loading symbols...</div>;
  const renderError = () => <div className="symbol-list error">Error: {error}</div>;
    ...
    <tbody>
      {symbols.map((symbol) => (
        <tr key={symbol.name}>
          ...
          <td>{symbol.last_price != null ? symbol.last_price.toFixed(2) : '-'}</td>
          <td>{symbol.last_price_date ?? '-'}</td>
          <td className="symbol-desc">{symbol.description ?? '-'}</td>
          <td>
            <button
  ...
  const renderDetailsView = () => {
    if (!current || !selectedSymbol) return null;
      ...
      <div className="symbol-list">
        ...
        <div className="symbol-details">
          <div className="symbol-details-header">
            <h2>{current.name}</h2>
            <button className="details-link" onClick={() => setDetailsView('dividends')}>
              Dividends
            </button>
          </div>
  ...
  const renderDividendsView = () => {
    ...
    return (
      <div className="symbol-list">
        <button className="back-btn" onClick={() => setDetailsView('details')}>
          ← Back
        </button>
        <div className="symbol-details">
          <div className="symbol-details-header">
            <h2>{current.name}</h2>
            <button className="details-link" onClick={() => setDetailsView('details')}>
              Details
            </button>
          </div>
  ...
  if (selectedSymbol) return detailsView === 'dividends' ? renderDividendsView() : renderDetailsView();
